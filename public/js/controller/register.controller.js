...
{   
    ...
    $scope.register=function()
    {
      ...
      if($scope.input_stat.email.error > 0 || 
         $scope.input_stat.password_confirm.error > 0 || 
         $scope.input_stat.password.error > 0 || 
         $scope.input_stat.fname.error > 0|| 
         $scope.input_stat.lname.error > 0)
        {
          console.log('error');
        }
      else
      {
        $scope.Load=true;
        $http.post('http://localhost/project/restaurant_level/public/register',{crfs:CSRF_TOKEN,data:$scope.data}).then(function successCallback(response) {
    ...
    $scope.interacted = function(field) {
      ...
      {
        $scope.input_stat.accepted.touched=field.$touched;
